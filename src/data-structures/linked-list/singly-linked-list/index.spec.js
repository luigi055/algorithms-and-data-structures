...
describe("Testing SinglyLinkedList Class", () => {
	...
	it("should create a linked list based on an iterator", () => {
		const linkedList = SinglyLinkedList.from([4, 18, 2, 9, 17]);

		expect(linkedList.get(0).value).toBe(4);
		expect(linkedList.get(1).value).toBe(18);
		expect(linkedList.get(2).value).toBe(2);
		expect(linkedList.get(3).value).toBe(9);
		expect(linkedList.get(4).value).toBe(17);

		const linkedList2 = SinglyLinkedList.from("4182917");

		expect(linkedList2.get(0).value).toBe("4");
		expect(linkedList2.get(1).value).toBe("1");
		expect(linkedList2.get(2).value).toBe("8");
		expect(linkedList2.get(3).value).toBe("2");
		expect(linkedList2.get(4).value).toBe("9");
		expect(linkedList2.get(5).value).toBe("1");
		expect(linkedList2.get(6).value).toBe("7");

		const set = new Set();
		set.add(4);
		set.add(18);
		set.add(12);
		const linkedList3 = SinglyLinkedList.from(set);

		expect(linkedList3.get(0).value).toBe(4);
		expect(linkedList3.get(1).value).toBe(18);
		expect(linkedList3.get(2).value).toBe(12);
	});
});
